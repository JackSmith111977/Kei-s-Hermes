...
// --- WebSocket 兼容层 ---
// 优先使用 ws 模块（支持 CDP 所需的额外选项），Node 22+ 原生 WebSocket 对 CDP 兼容性不足
let WS;
try {
  WS = (await import('ws')).default;
} catch {
  if (typeof globalThis.WebSocket !== 'undefined') {
    console.warn('[CDP Proxy] ws 模块未安装，回退到原生 WebSocket（可能有兼容性问题）');
    console.warn('  建议：npm install -g ws');
    WS = globalThis.WebSocket;
  } else {
    console.error('[CDP Proxy] 错误：未找到可用的 WebSocket 实现');
    console.error('  解决方案：npm install -g ws 或升级到 Node.js 22+');
    process.exit(1);
  }
}
...
async function getWebSocketUrl(port, wsPath) {
  ...
  // Chrome 147+ headless 需要带 UUID 的路径，通过 HTTP API 获取
  try {
    const res = await fetch(`http://127.0.0.1:${port}/json/version`);
    const data = await res.json();
    if (data.webSocketDebuggerUrl) {
      console.log(`[CDP Proxy] 从 /json/version 获取 WebSocket URL: ${data.webSocketDebuggerUrl}`);
