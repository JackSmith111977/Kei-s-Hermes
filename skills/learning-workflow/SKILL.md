...
| 核心脚本被覆盖/损坏 | 学习流程脚本（`learning-state.py`、`reflection-gate.py`、`skill_finder.py`）可能被意外覆盖为桩文件（已验证：被覆盖为 91 字节的无效 Python 代码，内容只剩 `print(f'{script_name} OK')`）。检测：检查文件大小是否 < 100 字节。恢复：`cp ~/.hermes/profiles/experiment/skills/learning-workflow/scripts/<script>.py ~/.hermes/skills/learning-workflow/scripts/<script>.py`。建议：每次学习开始前运行 `wc -l ~/.hermes/skills/learning-workflow/scripts/learning-state.py` 确认完整性（健康文件应有 700+ 行）。
| **skill_finder.py (v1) 对 dict 类型 triggers 崩溃** — 部分 SKILL.md 的 YAML frontmatter 中 `triggers` 字段包含 dict 对象（例如 `- type: deploy`），导致 `score_skill()` 中 `trigger.lower()` 报 `AttributeError: 'dict' object has no attribute 'lower'`。v2 (skill_finder_v2.py) 已正确处理 dict 且评分更优（TF-IDF + 同义词 + 标签匹配）。<br>• **修复**: v1 已添加 `isinstance(trigger, dict)` 检查。<br>• **推荐**: 优先使用 v2 — `python3 ~/.hermes/skills/learning-workflow/scripts/skill_finder_v2.py "<query>"`（阈值 50）。 |
