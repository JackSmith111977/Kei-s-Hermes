...
# ── 依赖链诊断 ──
sra dep-check                  # 检查 Gateway 依赖链健康度（避免孤儿配置）
```
...
3. 🗑️ **自动清理 Gateway drop-in**：删除 `~/.config/systemd/user/hermes-gateway.service.d/sra-dep.conf`（✅ 从 v1.2.1 起由 `cmd_uninstall()` 内置自动执行，无需手动操作）
4. 📦 `pip uninstall sra-agent -y`
5. 📁 `--all` 时删除 `~/.sra/`，否则提示保留
6. ⚠️ 系统级 systemd 服务（需 sudo）仅打印提示，不强行删除

**`sra dep-check` — 依赖链健康检查（v1.2.1+）：**
```bash
sra dep-check
# 输出示例（健康状态）：
#   📄 sra-dep.conf: 存在
...
#   🏠 srad.service: 不存在
#   ❌ 依赖链存在 1 个问题:
#      - sra-dep.conf 存在但 srad.service 不存在
```

用于诊断 SRA 与 Gateway 之间的 systemd 依赖是否健康。在升级前、卸载后、或怀疑孤儿配置时运行。
...
| **🔴 Gateway 启动失败: `srad.service not found`** | `systemctl --user start hermes-gateway` 报 `exit status 5` | drop-in `sra-dep.conf` 用了 `Requires=srad.service`，但 srad.service 单元已被删除 | 运行 `sra dep-check` 诊断；改 `Requires=` 为 `Wants=`；或 `sra uninstall --all` 自动清理 |
