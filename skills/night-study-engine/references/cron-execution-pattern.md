...
## ⚠️ execute_code Python 语法陷阱：f-string 与 dict 字面量中的引号冲突（2026-05-16 新增）

当在 `execute_code` 中构建带 `f-string` 引用的复杂 Python dict 字面量时，容易因外层字符串的引号混淆产生 `SyntaxError`。

### 真实案例

```python
# ❌ 错误 — 变量名内的引号冲突导致 SyntaxError
FIELD = "key_points"
new_concept = {
    FIELD": [        # ← SyntaxError: 引号在 FIELD 后中断
        "description"
    ]
}
...
### 根因
- `execute_code` 的 `code` 参数接收字符串，其中的引号与 Python 变量和 dict key 的语法交互
- 在快速迭代编写 JSON 形状的 dict 时，容易在变量名后误加引号（肌肉记忆使然）

### 缓解策略
1. **写完立即检查** dict 的 key-value 语法：`key:` 后不需要引号
2. **使用 `.setdefault()`** 而非整体赋值来避免复杂 dict 字面量
3. **分步构建**：先创建空 dict，再填充字段，而非一次性大块 dict 字面量
4. 若语法错误已发生，在第二次尝试时**复制正确的 dict 结构模板**而非从头手写
