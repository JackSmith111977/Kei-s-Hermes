...
def add_image(filepath, width=None, height=None, max_width=160*mm, caption=None):
    """添加图片到 PDF，可选标题"""
    from reportlab.lib.units import mm
    elements = []
    ...
    if caption:
        caption_style = S('Caption', fontSize=8, textColor=MEDIUM_GRAY, alignment=TA_CENTER, spaceAfter=3*mm)
        elements.append(Paragraph(esc(caption), caption_style))
    return elements
...
def build_pdf(output_path):
    doc = SimpleDocTemplate(
        ...
        creator='reportlab + pdf-layout skill v1.0.0',
    ...
    info_data = [
        [hp("版本", SB('Box', fontSize=9)), hp("v5.0.0", S('Box', fontSize=9))],
    ...
    # 封面配图 (DashScope 高清)
    story.extend(add_image("/tmp/hermes_pdf_images/project_overview.jpg", max_width=165*mm, caption="Hermes Agent - 多平台 AI 助手系统架构 (高清)"))
    ...
    # 六层架构配图
    story.extend(add_image("/tmp/hermes_pdf_images/six_layer_arch.jpg", max_width=165*mm, caption="六层模块化架构示意图"))
    ...
    # Agent 循环配图
    story.extend(add_image("/tmp/hermes_pdf_images/agent_loop.jpg", max_width=165*mm, caption="Agent 运行循环流程图"))
    ...
    # 工具系统配图
    story.extend(add_image("/tmp/hermes_pdf_images/tool_system.jpg", max_width=165*mm, caption="工具系统架构图"))
    ...
    # 记忆系统配图
    story.extend(add_image("/tmp/hermes_pdf_images/memory_system.jpg", max_width=165*mm, caption="记忆系统架构示意图"))
...
build_pdf(f'/home/ubuntu/.hermes/Hermes_Architecture_Guide_CN_v{__version__}.pdf')
